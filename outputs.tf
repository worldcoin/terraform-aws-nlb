output "ready" {
  ...
  value = {
    tls   = var.create_default_listeners ? "${aws_lb_listener.tls[0].arn}:${aws_lb_target_group[0].tls.id}" : ""
    plain = var.create_default_listeners ? "${aws_lb_listener.plain[0].arn}:${aws_lb_target_group[0].plain.id}" : ""
  }
}
